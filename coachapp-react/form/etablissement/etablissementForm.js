    ...
    return (
        <form onSubmit={e => onSubmit(e)} className={`${styles.card} ${styles.form}`}>
            ...
            <div className={styles.formComponent}>
                ...
                <input id={"heureOuverture"} type={"time"} max={heureFermeture} className={styles.formInput} onChange={e => setOuverture(e.target.value)}/>
                <span className={styles.error}>{errors["heureOuverture"]}</span>
            </div>

            <div className={styles.formComponent}>
                <label htmlFor={"heureFermeture"}>Heure de fermeture</label>
                <input id={"heureFermeture"} type={"time"} min={heureOuverture} className={styles.formInput} onChange={e => setFermeture(e.target.value)}/>
